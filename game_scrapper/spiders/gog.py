...
class GogSpider(Spider):
    ...
    def start_requests(self):
        yield Request(
            url=f"{BASE_URL}?limit=140&order=desc%3Atrending&productType=in%3Agame%2Cpack%2Cdlc%2Cextras&page=1&countryCode=BR&locale=en-US&currencyCode=BRL",
            callback=self.parse,
            meta={"page": 1},
        )
    ...
    def parse(self, response):
        ...
        if page < total_pages:
            yield Request(
                url=f"{BASE_URL}?limit=140&order=desc%3Atrending&productType=in%3Agame%2Cpack%2Cdlc%2Cextras&page={page + 1}&countryCode=BR&locale=en-US&currencyCode=BRL",
                callback=self.parse,
                meta={"page": page + 1},
            )